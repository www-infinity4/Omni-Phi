(() => {
  ...
  const keyOf = (item) => item?.storyKey || item?.image || item?.imageUrl || item?.sourceUrl || item?.url || item?.id || clean(item?.title, 180).toLowerCase().replace(/[^a-z0-9]+/g, '-');
  ...
  async function googleImagesSearch(term) {
    ...
    try {
      const response = await timeout(fetch(`https://r.jina.ai/${pageUrl}`, { cache: 'no-store', headers: { Accept: 'text/plain' } }), 8500);
      ...
      const add = (image, title = '') => {
        ...
        seen.add(image);
        out.push({ image, url: image, sourceUrl: pageUrl, title: title || q, alt: title || q, provider: 'Google Images', origin: 'google-images', googleSearchUrl: pageUrl });
      };
  ...
  async function openImagePanel() {
    ...
    const candidates = [...google, ...commons, ...openverse]
      ...
      .sort((a, b) => {
        const boost = (item) => item.origin === 'google-images' ? 80 : item.origin === 'commons' ? 55 : item.origin === 'openverse' ? 50 : 40;
        return (boost(b) + overlap(`${b.title || ''} ${b.alt || ''}`, query) * 8) - (boost(a) + overlap(`${a.title || ''} ${a.alt || ''}`, query) * 8);
      })
      .slice(0, 48);
    ...
    candidates.forEach((item) => {
      ...
      const alreadySelected = selectedKeys.has(candidateKey);
      const card = document.createElement('article');
      card.className = `omni-image-search-card${alreadySelected ? ' selected' : ''}`;
      card.innerHTML = `
      ...
      if (alreadySelected) selectButton.disabled = true;
      const select = () => {
  ...
  function ensureStyle() {
    if (document.getElementById('omni-image-search-v4-style')) return;
    const style = document.createElement('style');
    style.id = 'omni-image-search-v4-style';
    style.textContent = `
      .omni-image-stream-button{min-height:44px!important;padding:10px 14px!important;border:1px solid #ffd54a!important;border-radius:14px!important;background:#7c2d12!important;color:#fff7b8!important;font-weight:950!important;cursor:pointer}.omni-image-search-panel{margin:18px 0 28px;padding:16px;border:1px solid rgba(255,211,74,.48);border-radius:24px;background:linear-gradient(145deg,#17102c,#0a1b33);box-shadow:0 18px 42px rgba(0,0,0,.30);color:white}.omni-image-search-head small{color:#ffd54a;font-weight:950;letter-spacing:.13em}.omni-image-search-head h2{margin:.35rem 0 .5rem;color:white}.omni-image-search-head p{margin:0;color:#d9e3f0;line-height:1.55;max-width:52rem}.omni-image-nav{display:grid;grid-template-columns:minmax(0,1fr) auto minmax(0,1fr);align-items:center;gap:10px;margin-bottom:14px}.omni-image-nav.bottom{margin:16px 0 0}.omni-image-nav button,.omni-image-nav a{min-height:42px;padding:10px 12px;border:1px solid #ffd54a;border-radius:12px;background:#5b210d;color:#fff8c5;font-weight:900;text-decoration:none;display:flex;align-items:center;justify-content:center}.omni-image-nav a{background:#1f2937;border-color:#64748b}.omni-image-nav span{color:#ffd54a;font-weight:900;white-space:nowrap}.omni-image-search-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(165px,1fr));gap:12px;margin-top:16px}.omni-image-search-card{overflow:hidden;border:2px solid transparent;border-radius:17px;background:#0f172a;box-shadow:0 8px 24px rgba(0,0,0,.18)}.omni-image-search-card.selected{border-color:#ffd54a;box-shadow:0 0 0 3px rgba(255,213,74,.16),0 10px 26px rgba(0,0,0,.25)}.omni-image-search-card .image-pick{display:block;width:100%;padding:0;border:0;background:#020617;cursor:pointer}.omni-image-search-card img{display:block;width:100%;aspect-ratio:4/3;object-fit:cover}.omni-image-search-card>div{padding:10px;display:grid;gap:7px}.omni-image-search-card small{color:#80e8ff;font-weight:850}.omni-image-search-card strong{font-size:.84rem;line-height:1.3}.omni-image-search-card .select-image,.omni-image-search-card a{display:flex;align-items:center;justify-content:center;min-height:35px;border:1px solid #fb923c;border-radius:10px;background:#f97316;color:white;text-decoration:none;font:850 11px/1 system-ui,sans-serif}.omni-image-search-card.selected .select-image{background:#665b11;border-color:#ffd54a;color:#fff8c5}.omni-image-search-card a{background:#1e293b;border-color:#475569}.omni-image-search-loading{padding:20px;color:#d6deea;font-weight:800}.visual-pattern-note{margin:0 0 12px;padding:8px 11px;width:max-content;max-width:100%;border:1px solid #ffd54a;border-radius:999px;color:#fff3a3;background:#4b1f0b;font:850 .78rem/1.2 system-ui,sans-serif}@media(max-width:620px){.omni-image-nav{grid-template-columns:1fr}.omni-image-nav span{text-align:center}.omni-image-search-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
    `;
  ...
  function installButton() {
    ...
    if (old && old.dataset.v4 === '1') return true;
