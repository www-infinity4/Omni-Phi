...
function render(query,results,direction,visuals,tokenId,media){
 const p=infer(query),sources=sourceCards(results),stories=p.stories.length?p.stories.map(x=>({title:x[0],text:x[1]})):sources.slice(0,12),audio=(media||[]).filter(x=>x.type==="audio").map(x=>({title:x.title,text:String(x.description||"").slice(0,300),url:x.url})),video=(media||[]).filter(x=>x.type==="movies").map(x=>({title:x.title,text:String(x.description||"").slice(0,300),url:x.url}));
